...
- `scripts/player/DrillMovement.gd` (선택: 이동 벡터만 분리)
- `resources/player/drill_stats.tres` (선택: 속도, 가속 — 나중 업그레이드 대비)

**체크리스트**

- [ ] 입력에 따라 **의도 방향 벡터** 계산 (아래 = (0, 1), 좌 45° = 정규화된 (-1, 1) 등 — 좌표계에 맞게 조정)
- [ ] `move_and_slide` 등으로 충돌 처리, **타일 안으로 박힘** 방지
- [ ] “아래 입력은 마우스 클릭으로만” 명확히 (토글 vs 홀드 결정)
- [ ] A/D 동시 입력 시 우선순위 또는 합성 규칙 정함
- [ ] 드릴 위치를 월드 그리드에 **스냅할지** 여부 결정 (채굴 연동에 영향)
